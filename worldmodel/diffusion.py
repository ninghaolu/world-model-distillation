...
class Diffusion(nn.Module):
    ...
    def ddim_sample_step(
        ...
        if cfg != 1.0:
            v_pred_null = model(x, clipped_t, model.get_null_action(actions), cache_idx, start_frame, cache_type='null')
    ...
    def generate(
        ...
        while curr_frame < n_frames:
            ...
            for i in range(self.sampling_timesteps):
                ...
                v_null = model(x_pred[:, start_frame:], t_curr[:, start_frame:] * self.timesteps, model.get_null_action(actions)[:, start_frame:curr_frame+1], cache_type='null')
